...
RSpec.describe Services::CashRebate do
  let!(:user) { create(:user) }
  let!(:reward) { create(:reward, :cash_rebate) }

  subject { described_class }

  describe '#generatable?' do
    context 'generatable' do
      let!(:transactions) { create_list(:transaction, 10, user: user, amount: 10.00) }
    ...
    context 'non generatable' do
      let!(:transactions) { create_list(:transaction, 9, user: user, amount: 10.00) }
      it 'is false' do
  ...
  describe '.execute' do
    ...
    let(:meta_keys) { %w[source limit sum_amount].sort }
    ...
    it 'generate reward' do
      ...
      expect(user_reward.meta['source']).to eql('cash_rebate')
